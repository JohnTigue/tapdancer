  ...
  currentBeers.forEach(aCurrentBeer => {
    ...
    if(matchingPreviousBeer) {
      ...
      oldBeerCount++;
        ...
        aCurrentBeer.tapDateTime = moment().tz('America/Los_Angeles').format('M/DD[@]HH[h]');
...
let doYourThing = () => {                 
  return webFetch('http://cd.chucks85th.com/')
    ...
    .then(currentBeers => {
      ...
      if(!s3urls.valid(anUrl)) {
        ...
        return Promise.reject('bad S3 URL: ' + anUrl);
	      ...
	      console.log('JSON.parse("beers.json") ok: previousBeers.length = ' + previousBeers.length);
		...
		ACL:         'public-read'
	      ...
	      return putBeersDotJson
                .then(() => {
                  console.log('put to S3 worked. returning ' + currentBeers.length + ' beers');
		  return currentBeers;
                })
                .catch(err => {
    ...
    .then((currentBeers) => {
	  ...
	  currentBeers.nowDateTime = moment().tz('America/Los_Angeles').format('M/D[@]HH[:]mm'); 

	  return menuTemplate(currentBeers).then((menuRenderedString) => {
          //console.log(menuRenderedString);
            return fs.writeFileAsync(fileOpts.menuRenderedFilename, menuRenderedString, "utf8")
	      .then(() => {
                console.log('write menuRenderedString to FS: ok');
	        return menuRenderedString;
	      })
	      .catch(err => {
		console.error('ERROR fs.writeFileAsync(' + fileOpts.menuRenderedFilename + '): ' + err);
                return Promise.reject(err);		
		});
            });
          })
      ...
      return fs.readFileAsync(fileOpts.menuTemplateFilename, "utf8")
        .then((menuTemplateString) => {
          return new Promise(function(resolve, reject) {
            console.log('About to: Ftp.put(' + fileOpts.menuRenderedFilename + ', ' + ftpOpts.destFilename + ')' );
...
console.log('run invoked at ' + moment().tz('America/Los_Angeles').format('M/D[@]HH[:]mm'));
